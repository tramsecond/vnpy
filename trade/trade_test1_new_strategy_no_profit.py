...
def backtest_strategy(df, time_frame):
    """执行回测策略 - 止损策略：买入后根据信号和止损线决定卖出（无止盈线）"""
...
def backtest_single_file(file_path, output_dir):
    ...
    try:
        ...
        # 生成输出文件名
        output_filename = f"{file_name}_止损策略_回测结果.xlsx"
...
def generate_summary_report(all_stats, output_dir):
    ...
    try:
        ...
        summary_filename = f"止损策略汇总报告_{timestamp}.xlsx"
...
def main():
    ...
    output_dir = "backtest_results_stop_loss"
    ...
    for i, file_path in enumerate(excel_files, 1):
        print(f"\n[{i}/{len(excel_files)}] 回测止损策略: {os.path.basename(file_path)}")
