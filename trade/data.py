...
def log_error(file_name, sheet_name, reason, exception=None):
    ...
    # 写入日志文件
    with open("analysis_errors.log", "a", encoding="utf-8") as log_file:
...
def process_excel_file(file_path, output_dir):
    ...
    try:
        ...
        if all_signals:
            ...
            # 写入文件头
            worksheet.write(0, 0, "股票技术信号综合分析报告")
...
def main():
    ...
    if not excel_files:
        ...
        print(f"请确保数据准备脚本已在 '{data_dir}' 目录中生成了Excel文件")
        return
    ...
    # 初始化错误日志
    open("analysis_errors.log", "w", encoding="utf-8").close()
    ...
    if skipped_files:
        ...
        # 写入跳过文件日志
        with open("skipped_files.log", "w", encoding="utf-8") as log:
            log.write("\n".join(skipped_files))
    
    print("\n详细错误信息请查看: analysis_errors.log")
    print("跳过文件列表请查看: skipped_files.log")
    print("=" * 60)
